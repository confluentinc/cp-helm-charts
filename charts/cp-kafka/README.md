...
| `rackAware.enabled` | If enabled, copy the value of the node-label `failure-domain.beta.kubernetes.io/zone` as the `broker.rack` property. Will query kube-api for that info, so it needs service account and rbac enabled | `true` |
| `serviceAccount.enabled` | Create account to query kube-api  | `true` |
