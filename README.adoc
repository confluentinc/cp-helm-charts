...
helm repo update    #<2>
helm install my-confluent confluentinc/cp-helm-charts --version 0.6.0    #<3>
----
...
[source,sh]
----
helm install my-confluent-oss --set cp-schema-registry.enabled=false,cp-kafka-rest.enabled=false,cp-kafka-connect.enabled=false confluentinc/cp-helm-charts
----
...
----
helm upgrade <release name> confluentinc/cp-helm-charts
----
